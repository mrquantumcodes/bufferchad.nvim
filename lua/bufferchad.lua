...
M.opts = {}
M.marked = {}
M.marked_cache = {}  -- Cache for marked buffers per working directory

M.session_dir = vim.fn.stdpath('data'):gsub("\\", "/") .. "/marked_buffers/"
-- create the session directory if it doesn't exist
if not vim.uv.fs_stat(M.session_dir) then
	vim.fn.mkdir(M.session_dir, "p")
end


-- Load marked buffers from cache or file
local function load_marked_buffers()
	local cwd = vim.fn.getcwd()

	-- Return from cache if available
	if M.marked_cache[cwd] then
		return M.marked_cache[cwd]
	end

	-- Load from file
	local marksFile = M.session_dir .. M.pathToFilename(cwd)
	local marked = {}

	if vim.fn.filereadable(marksFile) == 1 then
		marked = vim.fn.readfile(marksFile)
		-- Remove empty lines
		local filtered = {}
		for _, v in ipairs(marked) do
			if v ~= "" then
	...
	-- Cache and return
	M.marked_cache[cwd] = marked
	return marked
end

-- Save marked buffers to cache and file
local function save_marked_buffers(marked)
	local cwd = vim.fn.getcwd()
	M.marked_cache[cwd] = marked
	M.marked = marked

	local marksFile = M.session_dir .. M.pathToFilename(cwd)
	vim.fn.writefile(marked, marksFile)
end

M.MarkedBuffersOpen = function(mode)
	M.marked = load_marked_buffers()
	M.OpenBufferWindow(M.marked, "Marked Buffers", "marked", mode)
end
...
M.filenameToPath = function(filename)
	...
	if vim.fn.isdirectory(decodeCheck) == 1 then
		...
		for _, value in ipairs(parts) do
			decodedG = decoded .. string.char(value)
		end
...
-- Function to get buffer info with last used time
...
M.BufferChadListBuffers = function()
	...
	-- REGULAR doesn't need sorting

	-- Swap first and second for LAST_USED_UP
	if sort_order == "LAST_USED_UP" and #buffers >= 2 then
		buffers[1], buffers[2] = buffers[2], buffers[1]
	elseif sort_order == "ASCENDING" then
		-- Reverse the table for ascending order
		local reversed = {}
...
M.OpenBufferWindow = function(buffer_names, title, mode, normalEditor)
	...
	if M.opts.style == "telescope" and not normalEditor then
		if pcall(require, 'telescope') then
			...
			print("Telescope is not installed, falling back to default")
		end
	end

	-- Default window style (also used for modern since we're removing dressing support)
	if true then
				...
				if selected_path then
					vim.api.nvim_buf_call(bufnr, function()
						...
						if mode == "marked" then
							...
							-- Save the buffer content as marked buffers
							save_marked_buffers(buffer_content)
						end
					end)
...
M.navigate_to_marked_buffer = function(position)
	-- Load marked buffers with caching
	M.marked = load_marked_buffers()

	local bufname = M.marked[position]
	if bufname and bufname ~= "" then
		vim.cmd('buffer ' .. bufname)
	elseif position > #M.marked or not bufname then
		print("No buffer found at position " .. position)
	end
end
...
vim.keymap.set('n', 'mdel', function()
	-- Load current marked buffers
	M.marked = load_marked_buffers()

	local markedBuffer = findMarkedBuffer(vim.fn.bufname(vim.fn.bufnr('%')):gsub("\\", "/"))
...
for i = 1, 9 do
	...
	vim.keymap.set('n', string.format('%dset', i), function()
		M.push_buffer_to_marked(1, vim.fn.line("."), i)
	end, { noremap = true, silent = true })
	vim.keymap.set('x', string.format('%dset', i), function()
		M.push_buffer_to_marked(1, vim.fn.line("."), i)
	end, { noremap = true, silent = true })

	-- Define the mappings for <N>swap
	vim.keymap.set('n', string.format('%dswap', i), function()
		-- Load current marked buffers
		M.marked = load_marked_buffers()
	...
	vim.keymap.set('n', string.format('%dnav', i), function()
		M.navigate_to_marked_buffer(i)
	end, { noremap = true, silent = true })
	vim.keymap.set('x', string.format('%dnav', i), function()
		M.navigate_to_marked_buffer(i)
	end, { noremap = true, silent = true })
end
...
M.nav_to_marked = function(n)
	...
	if M.marked[n] and M.marked[n] ~= "" then
		print("Navigating to buffer " .. M.marked[n])
		vim.cmd('edit ' .. M.marked[n])
	else
		print("No marked buffer at position " .. n)
	end
end
